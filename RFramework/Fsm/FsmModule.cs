...
namespace RFramework.Fsm
{
    ...
    /// 有限状态机管理器。
    /// </summary>
    internal sealed class FsmManager : RFrameworkModule, IFsmModule
    {
        ...
        public bool HasFsm(Type ownerType)
        {
            if (ownerType == null)
            {
                throw new RFrameworkException("Owner type is invalid.");
            }
        ...
        public IFsm<T> CreateFsm<T>(string name, T owner, params FsmState<T>[] states) where T : class
        {
            ...
            if (HasFsm<T>(name))
            {
                throw new RFrameworkException(Utility.Text.Format("Already exist FSM '{0}'.", typeNamePair));
            }
        ...
        public bool DestroyFsm<T>(IFsm<T> fsm) where T : class
        {
            if (fsm == null)
            {
                throw new RFrameworkException("FSM is invalid.");
            }
