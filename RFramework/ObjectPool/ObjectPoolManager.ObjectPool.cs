...
namespace RFramework.ObjectPool
{
    internal sealed partial class ObjectPoolManager : RFrameworkModule, IObjectPoolManager
    {
        ...
        {
            private readonly RFrameworkMultiDictionary<string, Object<T>> m_Objects;
            ...
            public ObjectPool(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority)
                : base(name)
            {
                m_Objects = new RFrameworkMultiDictionary<string, Object<T>>();
            ...
            public override int Capacity
            {
                ...
                set
                {
                    if (value < 0)
                    {
                        throw new RFrameworkException("Capacity is invalid.");
                    }
            ...
            public override float ExpireTime
            {
                ...
                set
                {
                    if (value < 0f)
                    {
                        throw new RFrameworkException("ExpireTime is invalid.");
                    }
            ...
            public void Register(T obj, bool spawned)
            {
                if (obj == null)
                {
                    throw new RFrameworkException("Object is invalid.");
                }
            ...
            public bool CanSpawn(string name)
            {
                if (name == null)
                {
                    throw new RFrameworkException("Name is invalid.");
                }

                RFrameworkLinkedListRange<Object<T>> objectRange = default(RFrameworkLinkedListRange<Object<T>>);
            ...
            public void Unspawn(object target)
            {
                if (target == null)
                {
                    throw new RFrameworkException("Target is invalid.");
                }
                ...
                {
                    throw new RFrameworkException(Utility.Text.Format("Can not find target in object pool '{0}', target type is '{1}', target value is '{2}'.", new TypeNamePair(typeof(T), Name), target.GetType().FullName, target));
                }
            }
            ...
            public void Release(int toReleaseCount, ReleaseObjectFilterCallback<T> releaseObjectFilterCallback)
            {
                ...
                {
                    throw new RFrameworkException("Release object filter callback is invalid.");
                }
            ...
            public override ObjectInfo[] GetAllObjectInfos()
            {
                ...
                foreach (KeyValuePair<string, RFrameworkLinkedListRange<Object<T>>> objectRanges in m_Objects)
                {
