...
namespace RFramework.ObjectPool
{
    ...
    internal sealed partial class ObjectPoolManager : RFrameworkModule, IObjectPoolManager
    {
        ...
        public bool HasObjectPool(Type objectType)
        {
            ...
            {
                throw new RFrameworkException(Utility.Text.Format("Object type '{0}' is invalid.", objectType.FullName));
            }
        ...
        public bool HasObjectPool(Predicate<ObjectPoolBase> condition)
        {
            if (condition == null)
            {
                throw new RFrameworkException("Condition is invalid.");
            }
        ...
        public bool DestroyObjectPool<T>(IObjectPool<T> objectPool) where T : ObjectBase
        {
            if (objectPool == null)
            {
                throw new RFrameworkException("Object pool is invalid.");
            }
        ...
        private IObjectPool<T> InternalCreateObjectPool<T>(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority) where T : ObjectBase
        {
            ...
            {
                throw new RFrameworkException(Utility.Text.Format("Already exist object pool '{0}'.", typeNamePair));
            }
