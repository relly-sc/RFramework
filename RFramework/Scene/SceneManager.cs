...
namespace RFramework.Scene
{
    ...
    /// 场景管理器。
    /// </summary>
    internal sealed class SceneManager : RFrameworkModule, ISceneManager
    {
        ...
        public void LoadScene(string sceneAssetName, int priority, object userData)
        {
            ...
            {
                throw new RFrameworkException(Utility.Text.Format("Scene asset '{0}' is being unloaded.", sceneAssetName));
            }

            if (SceneIsLoading(sceneAssetName))
            {
                throw new RFrameworkException(Utility.Text.Format("Scene asset '{0}' is being loaded.", sceneAssetName));
            }

            if (SceneIsLoaded(sceneAssetName))
            {
                throw new RFrameworkException(Utility.Text.Format("Scene asset '{0}' is already loaded.", sceneAssetName));
            }
        ...
        public void UnloadScene(string sceneAssetName, object userData)
        {
            ...
            if (!SceneIsLoaded(sceneAssetName))
            {
                throw new RFrameworkException(Utility.Text.Format("Scene asset '{0}' is not loaded yet.", sceneAssetName));
            }
        ...
        private void LoadSceneFailureCallback(string sceneAssetName, LoadResourceStatus status, string errorMessage, object userData)
        {
            ...
            throw new RFrameworkException(appendErrorMessage);
        }
        ...
        private void UnloadSceneFailureCallback(string sceneAssetName, object userData)
        {
            ...
            throw new RFrameworkException(Utility.Text.Format("Unload scene failure, scene asset name '{0}'.", sceneAssetName));
        }
    }
}
